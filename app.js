...
const RISK_COLORS = { Low: "#3F9D5E", Medium: "#D9A32B", High: "#E07B34", Critical: "#C6423F" };
...
const demoReports = [
  { id: "demo-1", token: "SM-DEMO-0001", category: "Broken streetlight", description: "Streetlight near the college gate is not working.", latitude: 19.1234, longitude: 72.9876, severity: "high", submittedAt: daysAgoIso(1, 21), status: "open", photo: null },
  { id: "demo-2", token: "SM-DEMO-0002", category: "Broken streetlight", description: "The same lane is dark after sunset.", latitude: 19.124, longitude: 72.9878, severity: "medium", submittedAt: daysAgoIso(2, 20), status: "under_review", photo: null },
  { id: "demo-3", token: "SM-DEMO-0003", category: "Open manhole", description: "Open manhole near the footpath, unmarked at night.", latitude: 19.1238, longitude: 72.9877, severity: "critical", submittedAt: daysAgoIso(3, 10), status: "open", photo: null },
  { id: "demo-4", token: "SM-DEMO-0004", category: "Waterlogging", description: "Water remains near the bus stop after rain.", latitude: 19.14, longitude: 72.99, severity: "medium", submittedAt: daysAgoIso(20, 12), status: "resolved", photo: null },
];
...
document.getElementById("use-location").addEventListener("click", () => {
  ...
  if (!navigator.geolocation) {
    status.textContent = "Geolocation is not supported on this device. Enter coordinates manually.";
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      status.textContent = "Could not get your location. Enter coordinates manually.";
    },
  );
});
...
document.getElementById("report-form").addEventListener("submit", (event) => {
  ...
  document.getElementById("location-status").textContent = "Enter coordinates or use your device location.";
...
function trackReport() {
  ...
  const submitted = new Date(report.submittedAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" });
...
function renderHome() {
  ...
  const hotspots = riskApi.groupReportsIntoHotspots(reports.filter((r) => r.status !== "resolved"), { groupingRadiusMeters: GROUPING_RADIUS });
...
function drawMap() {
  ...
  visible.forEach((report) => {
    const nearby = visible.filter((r) => r.id !== report.id && riskApi.distanceInMeters(report.latitude, report.longitude, r.latitude, r.longitude) <= GROUPING_RADIUS);
    const risk = riskApi.calculateRiskScore(report, nearby);
...
function renderDashboard() {
  ...
  const activeHotspots = riskApi.groupReportsIntoHotspots(reports.filter((r) => r.status !== "resolved"), { groupingRadiusMeters: GROUPING_RADIUS });
  const highRisk = activeHotspots.filter((h) => h.riskLevel === "High" || h.riskLevel === "Critical").length;
